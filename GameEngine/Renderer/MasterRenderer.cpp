...
namespace RenderM
{
	MasterRenderer::MasterRenderer()
		: _entity_shader(new ShaderM::StaticShader), _terrain_shader(new ShaderM::TerrainShader)
	{
		_entities = new map<ModelM::Model*, vector<EntityM::Entity*>>;
		enableCulling();
		glCullFace(GL_BACK);

		createProjectionMatrix();
		_terrains = new vector<TerrainM::Terrain*>();
	...
	void MasterRenderer::processTerrain(TerrainM::Terrain* terrain)
	{
	...
	void MasterRenderer::processEntity(EntityM::Entity* entity)
	{
		assert(entity != NULL);
		/*
		...
		*/
		...
		ModelM::Model* model = entity->model();
		if (_entities->empty())
		{
			vector<EntityM::Entity*>* t = new vector<EntityM::Entity*>();
			t->push_back(entity);
