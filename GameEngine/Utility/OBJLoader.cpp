...
namespace UtilityM
{
	...
	ModelM::ModelMesh * OBJLoader::default_model = nullptr;

	ModelM::ModelMesh * OBJLoader::getDefaultModel()
	{
		if (!OBJLoader::default_model)
		{
			OBJLoader::default_model = OBJLoader::loadOBJ(OBJLoader::default_model_filename);
		}
	...
	//
	ModelM::ModelMesh * OBJLoader::loadOBJ(	string fileName	)
	{
		...
		ModelM::ModelMesh* m =  Loader::loadToVao(out_vertices, out_uvs, out_normals, indices);
